...
for useImageSet = 4 % 1:6  or just choose one image set, e.g. 3
  switch useImageSet
   case 1
    name = 'gray';
   case 2
...
%   continue;
  ...
  % Read in images, store 3-channel vectorized images.
  imData_3ch = zeros(numPixels, 3, nDir);
  for i=1:nDir
    fname = [imageDir,name,'.',num2str(i-1),'.ppm'];
    im = ppmRead(fname);
    imData_3ch(:,:,i) = reshape(im, [numPixels, 3]);
  end
  
  % Compute albedo for 3 channels
  imData_3ch = imData_3ch(:,:,1:8);    % (nPixel, 3, 8)
  dividend = zeros(size(albedo));
  divider = dot(n * L * L', n, 2);
  for v=1:3
      I = squeeze(imData_3ch(:,v,:));  % (nPixel, 8)
      dividend(:,v) = dot(n*L, I, 2);
      albedo(:,v) = dividend(:,v) ./ divider;
  end
  ...
  I = imData(mask,(nDirChrome+1):nDir);
  Lrec = pinv(g' * g) * g' * I;
